 ...
 * 修改内容：
 */
package com.dream.util.excelTools;
...
public class JsGridReportBase {
	 ...
	 * @return void
	 */
	private void stopGrouping(HSSFSheet sheet, HashMap<Integer, Object> word,
	...
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData) throws Exception {
		...
		for (TableDataRow dataRow : dataRows) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (!tc.isVisible())
					continue;
				index++;
				Object value = dataCells.get(i).getValue();
				if (tc.isGrouped()) {
		...
		for (int c = 0; c < headerMetaData.getColumns().size(); c++) {
			...
			if(sheet.getColumnWidth(c)<t.length()*256*4)
				sheet.setColumnWidth(c, t.length()*256*4);
		}
...
//			fileNames = new String(sFileName.getBytes("UTF-8"), "GBK"); // 处理中文文件名的问题
	...
	private void createCell(HSSFRow row, TableColumn tc,
		...
		default:
			...
			System.out.println("STRING"+dc.getValue());
			if (style != null)
				cell.setCellStyle(style);
		}*/
	...
	private HashMap<String, HSSFCellStyle> initStyles(HSSFWorkbook wb) {
		...
		try {
			ClassPathResource res = new ClassPathResource("module.xls", this.getClass().getClassLoader());//注意：module.xls模板文件跟该类同一路径
			...
			buildStyle(wb, src, sheet, 10, ret, "STRING");//字符串单元格样式（带背景色）
			buildStyle(wb, src, sheet, 11, ret, "INT");//整数单元格样式（带背景色）
			buildStyle(wb, src, sheet, 12, ret, "D2");//2位小数单元格样式（带背景色）
			buildStyle(wb, src, sheet, 13, ret, "D3");//3位小数单元格样式（带背景色）

			buildStyle(wb, src, sheet, 15, ret, "RED");//红色单元格背景
			buildStyle(wb, src, sheet, 16, ret, "YELLOW");//黄色单元格背景
			buildStyle(wb, src, sheet, 17, ret, "GREEN");//绿色单元格背景*/
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ret;
