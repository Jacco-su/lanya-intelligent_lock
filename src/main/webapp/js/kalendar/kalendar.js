	...
	function kalendarCode(codeYear,codeMonth,codeDay,codeFirst){
		...
		for(var m=0;m<6;m++){//日期共 4-6 行
			...
			if(m >= Math.ceil((codeFirst+monthDays[codeMonth])/7)){//第五、六行是否隐藏				
				...
				kalendar_html += "<ul class='dayList dayListHide"+m+"' id='dateul'>\n";
			}
			var datevalue='';
			for(var n=0;n<7;n++){//列
			    datevalue='';
				...
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					kalendar_html += "<li datevalue='' id='li"+liid+"'><p></p></li>";
				}else{
				    datevalue=(7*m+n+1-codeFirst);
					var liDateValue="";
					if(datevalue<10){
						liDateValue=liDateValue+"0"+datevalue;
					}else{
						liDateValue=liDateValue+""+datevalue;
					}
					if((7*m+n+1-codeFirst == codeDay)&&(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6))){//当天是周末
						kalendar_html += "<li class='todayWeekend' datevalue='"+datevalue+"' id='li"+liid+"'><p class='showdate'>"+datevalue+"</p><p name='content' class='content'></p></li>";
					}else if(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6)){//仅是周末
					    datevalue=(7*m+n+1-codeFirst);
						kalendar_html += "<li class='weekend' datevalue='"+datevalue+"' id='li"+liid+"'><p class='showdate'>"+datevalue+"</p><p name='content' class='content'></p></li>";
					}else if(7*m+n+1-codeFirst == codeDay){//仅是当天
					    datevalue=(7*m+n+1-codeFirst);
						kalendar_html += "<li class='today' datevalue='"+datevalue+"' id='li"+liid+"'><p class='showdate'>"+datevalue+"</p><p name='content' class='content'></p></li>";
					}else{//其他日期
						datevalue=(7*m+n+1-codeFirst);
						kalendar_html += "<li datevalue='"+datevalue+"' id='li"+liid+"'><p class='showdate'>"+datevalue+"</p><p name='content' class='content'></p></li>";
					}
				}
			}
			kalendar_html += "</ul>\n";
		...
		*/
		...
		for(var i=0;i<7*newLine;i++){
			...
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
				...
				dateObj.eq(i).html("<p class='showdate'>"+(i+1-dateFirstDay)+"</p><p name='content' class='content'></p>");
				dateObj.eq(i).attr("datevalue",(i+1-dateFirstDay));
			}
		}
		findByYearMonth();
	}
...
/*************    缓存节点和变量     **************/
	...
	var yearfloor = 3;//选择年份从1970到当前时间的后10年
...
/***************************/
	...
	var dateDetail="";
	dateDay.bind('dblclick',function(){
		dateDetail="";
		iscontent=false;
		var selValue=$(this).attr("datevalue");
		if(selValue==''){
			return;
		}
		dateDetail=""+yearChange;
		var monthInt=parseInt(monthChange)+1;
		if(monthInt<10){
			dateDetail=dateDetail+"-0"+monthInt;
		}else{
			 dateDetail=dateDetail+"-"+monthInt;
		}
		var dayInt=parseInt(selValue);
		if(dayInt<10){
			dateDetail=dateDetail+"-0"+dayInt;
		}else{
			dateDetail=dateDetail+"-"+dayInt;
		}
		var sel_liid=$(this).attr("id");
		
		prAdd(dateDetail,sel_liid);
	});
	
	function findByYearMonth(){
				var yearMonth=""+yearChange;
				var monthInt=parseInt(monthChange)+1;
				if(monthInt<10){
					yearMonth=yearMonth+"-0"+monthInt;
				}else{
					yearMonth=yearMonth+"-"+monthInt;
				}
		        $.post(url,{"yearMonth":yearMonth,"userId":options.userId},function(datas){
		        						var rowsList=datas.rows;
		        						dateDay.each(function(){
		        							  		var liidValue=$(this).attr("id");
		        							  		var lidatevalue=$(this).attr("datevalue");
		        							  		if(lidatevalue==""){
		        							  			return;
		        							  		}
		        							  		var yearMonthDate=yearMonth;
		        									var dayInt=parseInt(lidatevalue);
		        									if(dayInt<10){
		        										yearMonthDate=yearMonthDate+"-0"+dayInt;
		        									}else{
		        										yearMonthDate=yearMonthDate+"-"+dayInt;
		        									}
		        									var contents="";
		        									var dateListCount=0;
		        									for(var j=0;j<rowsList.length;j++){
		        										      var affair=rowsList[j];
		        										      if(affair.startTime==yearMonthDate){
		        										    	      if(dateListCount>=3){
		        										    	    	       contents=contents+"<a href='javascript:void(0)' onclick=\"showMore('"+yearMonthDate+"')\">更多</a><br/>";
		        										    	    	       break;
		        										    	      }
		        										    	       var theContent=affair.title;
		        										    	  		contents=contents+"<a href='javascript:void(0)' onclick=\"showContent('"+affair.id+"')\">"+theContent+"</a><br/>";
		        										    	  		dateListCount++;
		        										      }
		        									}
		        									//$("#"+liidValue+" p[name='content']").html(contents);
		        									$(this).find("p[name='content']").html(contents);
		        						});
		        },"json");
	}
