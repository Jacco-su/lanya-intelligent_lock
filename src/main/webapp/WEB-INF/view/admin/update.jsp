...
<table class="mytable" align="center">
	<tr>
		...
		<td>
			...
			<input name="name" readonly="readonly" class="easyui-validatebox" required="true" value="${user.name}" size="40"/>
...
<%--
	...
	<tr>
		<td>手机:</td>
		<td><input name="phone" size="40" class="easyui-validatebox"  validType="mobile" value="${user.phone}"/></td>
	</tr>
	<tr>
		<td>Email:</td>
		<td><input name="email" size="40" class="easyui-validatebox"  validType="email" value="${user.email}"/></td>
	</tr>
	<tr>
		<td>排序序号:</td>
		<td>
	...
	<tr>
		...
		<td>
			...
			<c:forEach items="${roles}" var="role" varStatus="s">
				<td style="border: 0px;">
					<input name="roIdList" type="checkbox" value="${role.roId}"/>${role.name }
				</td>
				<c:if test="${(s.index+1)%3==0}"></tr><tr></c:if>
			</c:forEach>
			</tr>
			</table>
		</td>
